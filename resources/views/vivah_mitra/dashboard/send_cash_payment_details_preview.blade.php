                    ...
                    <div class="features-box  mt-3">
						...
						<div class="row m-b20 g-3">
							<div class="container">
								...
								<form method="post"  autocomplete="off" id="save-form" action="{{ route('member.cash.final.save') }}"  enctype="multipart/form-data">
									@csrf
									...
									<input type="hidden" name="temp_id" value="{{ $entry->id }}">
									<div class="row formtype">
										...
										<div class="col-md-12 mb-3">
											<div class="form-group">
												...
												<input required type="date" name="date" id="date" placeholder="Enter Amount" value="{{ date('Y-m-d') }}" class="form-control" readonly>
											</div>
										</div>
										...
										<div class="col-md-12 mb-3">
											<div class="form-group">
												...
												<input required type="" id="date" name="day" placeholder="Enter Amount" value="{{ date('l') }}" class="form-control" readonly>
											</div>
										</div>
												...
												<tr>
													<td>
														<input type="text" class="form-control"
															   name="notes[{{ $detail->note_value }}][note]"
															   value="{{ $detail->note_value }}" readonly>
													</td>

													<td>
														<input type="number" class="form-control qty"
															   data-note="{{ $detail->note_value }}"
															   name="notes[{{ $detail->note_value }}][qty]"
															   value="{{ $detail->quantity }}">
													</td>

													<td>
														<input type="text" class="form-control total"
															   name="notes[{{ $detail->note_value }}][total]"
...
$(document).ready(function () {
	...
	$(document).on('click', '.saveDetails', function(e) {
        ...
        $.ajax({
            type: "POST",
			url: "{{ route('member.cash.final.save') }}",
			data: formData,
			processData: false,
			contentType: false,
			dataType: "JSON",
			success: function(data) {
                ...
                if (data.status == true) {
					...
					window.location = "{{ url('member/cash-payment-send-details') }}";
