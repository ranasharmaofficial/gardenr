@extends('vivah_mitra.layouts.master')
@section('title', 'Group Notification')

@section('meta_tags')
@endsection

@section('content')
<style>
.message-wrapper{
    padding:15px;
    display:flex;
    flex-direction:column;
    gap:14px;
}

/* Card */
.message-card{
    display:flex;
    align-items:flex-start;
    gap:12px;
    background:#fff;
    border-radius:16px;
    padding:14px 16px;
    box-shadow:0 8px 20px rgba(0,0,0,0.08);
    transition:.25s;
}

.message-card:hover{
    transform:translateY(-3px);
}

/* Bell Icon */
.msg-icon{
    width:42px;
    height:42px;
    border-radius:12px;
    background:linear-gradient(135deg,#c7902f,#f9d976);
    display:flex;
    align-items:center;
    justify-content:center;
    color:#fff;
    font-size:16px;
}

/* Text */
.msg-text{
    font-size:14px;
    font-weight:500;
    color:#333;
}

/* Time */
.msg-time{
    font-size:12px;
    color:#888;
    margin-top:5px;
}


.message-card{
    display:flex;
    justify-content:space-between;
    align-items:flex-start;
    gap:12px;
    padding:14px 16px;
    background:#fff;
    border-radius:16px;
    box-shadow:0 6px 18px rgba(0,0,0,0.08);
}

/* left content */
.msg-left{
    flex:1;
}

/* message */
.msg-text{
    font-size:14px;
    color:#333;
}

/* time */
.msg-time{
    font-size:12px;
    color:#999;
    margin-top:6px;
}

/* copy button */
.msg-actions{
    display:flex;
    gap:8px;
}

/* copy */
.msg-copy{
    width:34px;
    height:34px;
    background:#f2f3f7;
    border-radius:10px;
    display:flex;
    align-items:center;
    justify-content:center;
    cursor:pointer;
}

/* whatsapp */
.msg-wa{
    width:34px;
    height:34px;
    background:#25D366;
    color:#fff;
    border-radius:10px;
    display:flex;
    align-items:center;
    justify-content:center;
    cursor:pointer;
}

/* hover */
.msg-copy:hover{ background:#ffd54f; }
.msg-wa:hover{ transform:scale(1.05); }
</style>

    <header class="header">
        <div class="main-bar">
            <div class="container">
                <div class="header-content">
                    <div class="left-content">
                        <a href="{{ url()->previous() }}" class="back-btn">
                            <svg width="18" height="18" viewBox="0 0 10 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M9.03033 0.46967C9.2966 0.735936 9.3208 1.1526 9.10295 1.44621L9.03033 1.53033L2.561 8L9.03033 14.4697C9.2966 14.7359 9.3208 15.1526 9.10295 15.4462L9.03033 15.5303C8.76406 15.7966 8.3474 15.8208 8.05379 15.6029L7.96967 15.5303L0.96967 8.53033C0.703403 8.26406 0.679197 7.8474 0.897052 7.55379L0.96967 7.46967L7.96967 0.46967C8.26256 0.176777 8.73744 0.176777 9.03033 0.46967Z"
                                    fill="#a19fa8"></path>
                            </svg>
                        </a>
                    </div>

                    <div class="mid-content">
                        <h5 class="mb-0">ग्रुप सूचना</h5>
                    </div>

                    <div class="right-content">
                        <a href="javascript:void(0);" class="menu-toggler">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path opacity="0.4"
                                    d="M16.0755 2H19.4615C20.8637 2 22 3.14585 22 4.55996V7.97452C22 9.38864 20.8637 10.5345 19.4615 10.5345H16.0755C14.6732 10.5345 13.537 9.38864 13.537 7.97452V4.55996C13.537 3.14585 14.6732 2 16.0755 2Z"
                                    fill="#a19fa8"></path>
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M4.53852 2H7.92449C9.32676 2 10.463 3.14585 10.463 4.55996V7.97452C10.463 9.38864 9.32676 10.5345 7.92449 10.5345H4.53852C3.13626 10.5345 2 9.38864 2 7.97452V4.55996C2 3.14585 3.13626 2 4.53852 2ZM4.53852 13.4655H7.92449C9.32676 13.4655 10.463 14.6114 10.463 16.0255V19.44C10.463 20.8532 9.32676 22 7.92449 22H4.53852C3.13626 22 2 20.8532 2 19.44V16.0255C2 14.6114 3.13626 13.4655 4.53852 13.4655ZM19.4615 13.4655H16.0755C14.6732 13.4655 13.537 14.6114 13.537 16.0255V19.44C13.537 20.8532 14.6732 22 16.0755 22H19.4615C20.8637 22 22 20.8532 22 19.44V16.0255C22 14.6114 20.8637 13.4655 19.4615 13.4655Z"
                                    fill="#a19fa8"></path>
                            </svg>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </header>

    @include('vivah_mitra.includes.sidebar')

    <div class="page-content">
        <div class="content-inner pt-0">
            <div class="container fb">

                <div class="dashboard-area m-b30">
                    <div class="features-box mt-3">
                        <div class="row m-b20 g-3">

							<img style="margin-top: -10px;" src="{{ static_asset('assets/assets_vivah_mitra/images/sandesh-image.png') }}" >

							<div class="message-wrapper">

								@foreach($notification_list as $msg)
								<div class="message-card">

									<div class="msg-left">
										<div class="msg-text">
											{{ $msg->messages }}
										</div>

										<div class="msg-time">
											{{ \Carbon\Carbon::parse($msg->created_at)->diffForHumans() }}
										</div>
									</div>

									<!-- Right actions -->
									<div class="msg-actions">

										<!-- Copy -->
										<div class="msg-copy" onclick="copyMsg(this)">
											<i class="fa fa-copy"></i>
										</div>

										<!-- WhatsApp -->
										<div class="msg-wa" onclick="shareWhatsApp(this)">
											<i class="fa-brands fa-whatsapp"></i>
										</div>

									</div>

								</div>

								@endforeach

							</div>

                             

                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>
<script>
// COPY
function copyMsg(el){
    const text = el.closest('.message-card')
                   .querySelector('.msg-text')
                   .innerText;

    navigator.clipboard.writeText(text);

    el.innerHTML = '<i class="fa fa-check"></i>';

    setTimeout(()=>{
        el.innerHTML = '<i class="fa fa-copy"></i>';
    },1000);
}


// WHATSAPP SHARE
function shareWhatsApp(el){

    const text = el.closest('.message-card')
                   .querySelector('.msg-text')
                   .innerText;

    const url = "https://wa.me/?text=" + encodeURIComponent(text);

    window.open(url, '_blank');
}
</script>
    @include('vivah_mitra.includes.home_footer_menu')

@endsection