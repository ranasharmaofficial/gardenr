...
<div class="main-content app-content">
    ...
    <div class="page-box">
        ...
        <form method="POST" action="{{ route('admin.assign.shop.store') }}">
            @csrf

            <input type="hidden" name="user_id" value="{{ $staff->id }}">

            <h4>
                <label style="background-color:#ffff0026;padding:8px 12px;border-radius:5px;">
                    Assign Shop
                </label>
            </h4>

            <div class="district-wrapper">
                @foreach($shop_list as $district)
                    <label class="district-item">
                        <input type="checkbox"
                               name="shops[]"
                               value="{{ $district->id }}"
                               {{ $staff->shops->contains($district->id) ? 'checked' : '' }} >

                        <span class="checkmark"></span>
                        <b>{{ $district->name }}</b>   &nbsp;&nbsp;&nbsp;(<b>{{ $district->investor_name }}</b>)
                    </label>
                @endforeach
            </div>
