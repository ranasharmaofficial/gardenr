								...
								<div class="card card-table">
									...
									<div class="col-md-3 mb-4">
										...
										<div class="table-responsive">
											<div id="user-table">
												@include('admin.staffs.vivah_mitra.vivahmitra.user_table', ['users' => $users])
											</div>
        ...
        <form method="post" enctype="multipart/form-data" id="card-given-form" action="">
			...
			<div class="modal-body">
				...
				<div class="row">
					...
					<div class="form-group">
						<label>Aayushmati Membership Card Value</label>
...
<div id="giveKitCardModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
	<!-- Modal content-->
    <div class="modal-content">
		  <div class="modal-header bg-danger text-white">
			<h4 class="modal-title">Issue Kit Package</h4>&nbsp;&nbsp;
			<button type="button" style="color:#fff;" class="close btn-primary" data-bs-dismiss="modal">X</button>
		  </div>
        <form method="post" enctype="multipart/form-data" id="kit-given-form" action="">
			@csrf
			<div class="modal-body">
				<div class="row">
					<div style="display:none;" id="show-kit-form-error" class="alert alert-danger col-md-12">
						<ul>
							<div class="errorMsgntainer"></div>
						</ul>
					</div>
				...
				<div class="row">
					<div class="col-md-12">
						<h4>Name: <span class="text-success" id="staff_name6"></span></h4>
						<h4>Employee Code: <span class="text-success" id="employee_code6"></span></h4>
					</div>
					
					 
					 
					<input type="hidden" id="staff_id6" name="user_id" class="form-control" required>
					 
					<div class="form-group">
						<label>Enter No. of Cards Given</label>
						<input name="quantity" id="kit_given" type="number" class="form-control">
					</div>
			...
			<div class="modal-footer">
				<button type="submit" class="btn btn-info save_kit_given" id="">SAVE</button>
				<button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
			</div>
        </form>
    </div>
...
<script>
	...
	/* update VM Box*/
	$(document).on('submit', '.update-vm-box-form', function(e) {
    ...
    let button = td.find('.update_vm_button');

    let errorBox = td.find('.show-vm-box-form-error');

    let errorContainer = td.find('.errorMsgContainer');

    button.prop('disabled', true).text('Updating...');

    $.ajax({
        url: "{{ route('admin.updateVMBox') }}",
        type: "POST",
        data: form.serialize(),
        dataType: "json",
	...
	function giveKitCard(giveKitCard){
        $('#giveKitCardModal').modal('show'); 
        let staff_ids = $(giveKitCard).attr('id');
		var base_url = "{{ url('admin/emp/get-employee-details/') }}";
		$.ajaxSetup({
			headers: {
				'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $.ajax({
           url: base_url,
           type: 'post',
           data:'staff_id='+staff_ids,
           success:function(response){
			  //console.log(response.data.id);
              $('#staff_id6').val(response.data.id);
              $('#staff_name6').text(response.data.first_name);
              $('#employee_code6').text(response.data.employee_code);
			}
       })
    }
	
	$(document).ready(function () {
        $('#card_given').on('keyup change', function () {

            let qty = parseInt($(this).val()) || 0;
            let pricePerCard = 999;

            let total = qty * pricePerCard;
	...
	/* kit modal */
	$(document).on('submit', '#kit-given-form', function(e) {
		e.preventDefault();
		var clk_btns = $(".save_kit_given");
		clk_btns.prop('disabled', true).text('Saving...');
		var formData = new FormData(this);
		$.ajaxSetup({
			headers: {
		...
		$.ajax({
			type: "POST",
			url: "{{ route('admin.emp.saveKitGiven') }}",
			data: formData,
			processData: false,
			contentType: false,
			dataType: "JSON",
			success: function(data) {
				...
				if (data.status === true) {
					$('#kit-given-form')[0].reset();
					document.getElementById('show-kit-form-error').style.display = "none";
					$('.errorMsgntainer').html('');
					// $('#updateStaffPhotoModal').modal('hide'); 
					Swal.fire({
						position: "top-end",
			...
			error: function(err) {
				clk_btns.prop('disabled', false).text('SAVE');
				document.getElementById('show-kit-form-error').style.display = "block";
