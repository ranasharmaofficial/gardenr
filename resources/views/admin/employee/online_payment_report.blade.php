...
@section('title', 'Online Payment Report List')
@section('content')
...
<div style="background:linear-gradient(45deg, #f33057, rgb( 56, 88, 249 ) );"  class="d-sm-flex d-block align-items-center justify-content-between page-header-breadcrumb">
    <div>
        <h4 class="fw-medium mb-2">{{ $page_title }} List</h4>
        <div class="ms-sm-1 ms-0">
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="javascript:void(0);">{{ $page_title }}</a></li>
                    <li class="breadcrumb-item active fw-normal" aria-current="page">Online Payment Sent Report</li>
                </ol>
            </nav>
        </div>
    </div>
								...
								<div class="card card-table">
									...
									<div class="card-body booking_card">
										...
										<div class="table-responsive">
											<div id="user-table">
												@include('admin.employee.partials.online_payment_report_table_ajax', ['online_payments' => $online_payments])
											</div>
...
$(document).ready(function () {
    ...
    // Open Modal
    $(document).on('click', '.paymentStatusBtn', function () {
    ...
    $('#saveStatusBtn').click(function () {
        ...
        $.ajax({
            url: "{{ route('admin.online.payment.status.update') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
    ...
    function fetchUsers(page = 1) {
        ...
        $.ajax({
            url: "{{ route('admin.staff.fetchOnlinePaymentReport') }}?page=" + page,
            method: "GET",
            data: {
                user_id,
                search
