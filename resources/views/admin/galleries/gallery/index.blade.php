							...
							<form method="GET" class="form mt-3 mb-3">
									...
									<div class="col-md-4">
										<select class="form-control" name="category_id">
											...
											@foreach($categories as $category)
												<option value="{{$category->id}}" @if($request->category_id == $category->id) selected @endif>{{$category->title.' - '.$category->description}}</option>
											@endforeach
										</select>
									</div>
									<div class="col-md-4">
                                ...
                                <table id="responsiveDataTable" class="table table-bordered text-nowrap mt-3" style="width:100%">
                                    ...
                                    <tbody>
										...
										<tr>
											<td>{{ $key + 1 }}</td>
											<td>{{ $value->parent_name }} - {{ $value->cat_description }}</td>
