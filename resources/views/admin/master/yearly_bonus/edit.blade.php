                    ...
                    <div class="col-xl-12">
                        <div class="card custom-card">
                            <div class="card-header d-flex justify-content-between align-items-center">
								...
								<a href="{{ url('admin/master-yearly-bonus-list') }}" class="btn btn-danger btn-sm float-end">
								   {{ $page_title }} List
								</a>
									...
									<div class="row">
											...
											<div class="form-group">
												...
												<select class="form-control" name="type" id="type">
													...
													@foreach($user_type as $val)
														<option @if($bonus->user_type==$val->id) selected @endif value="{{ $val->id }}">{{ $val->name }}</option>
													@endforeach
												</select>
											</div>
										</div>
										...
										<div class="col-md-6 mb-3">
											<div class="form-group">
												...
												<select class="form-control" name="user_designation_id" id="user_designation_id">
													<option @if($bonus->user_designation==$user_designation_details->id) selected @endif value="{{ $user_designation_details->id }}">{{ $user_designation_details->name }}</option>
												</select>
											</div>
										</div>

										<input type="hidden" value="{{ $bonus->id }}" id="agreement_id">

										<div class="col-md-6 mb-3">
											<div class="form-group">
												<label class="">Title</label>
												<input type="text" value="{{ $bonus->title }}" name="title"  id="agreement_title" class="form-control" placeholder="Agreement Title">
											</div>
										</div>
										...
										<div class="col-md-6 mb-3">
											<div class="form-group">
												...
												<input type="number" value="{{ $bonus->value }}" name="value"  id="agreement_title" class="form-control" placeholder="Agreement Title">
											</div>
										</div>
										...
										<div class="col-md-6 mb-3">
											<div class="form-group">
												...
												<select id="agreement_status" name="status" class="form-control">
													<option @if($bonus->status==1) selected @endif  value="1">Active</option>
													<option @if($bonus->status==0) selected @endif  value="0">Inactive</option>
												</select>
											</div>
										</div>
...
<script>
	...
	$(document).on('click', '.updateAgreementBtn', function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "{{ url('admin/yearlyBonus/update') }}/" + id,
            data: formData,
            processData: false,
            contentType: false,
            dataType: "JSON",
