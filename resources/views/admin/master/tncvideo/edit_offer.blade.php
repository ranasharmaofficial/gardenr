									...
									<div class="row">
										...
										<div class="col-md-6 mb-3">
											<div class="form-group">
												...
												<input type="text" name="video_url" id="agreement_file" value="{{ $agreement->video_url }}" class="form-control" placeholder="Video Url">
												<a target="_blank" class="btn btn-primary btn-sm mt-3" href="{{ $agreement->video_url }}">Check Current Video</a>
											</div>
										</div>
...
<script>
	...
	$(document).on('click', '.updateAgreementBtn', function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "{{ url('admin/tndcVid/update') }}/" + id,
            data: formData,
            processData: false,
            contentType: false,
            dataType: "JSON",
