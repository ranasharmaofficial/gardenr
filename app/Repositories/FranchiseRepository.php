...
class FranchiseRepository implements FranchiseRepositoryInterface
{
    ...
    public function storeStudentDetails($data){
        ...
        $students->roll_number = $data['roll_number'];
        $students->serial_number = $data['serial_number'];
    ...
    public function getStudentList($user_id,$status){
        return Student::where('franchise_id', $user_id)->where('status', $status)->orderBy('id', 'DESC')->get();
    }
