...
use App\Models\MasterKitNumber;
use App\Models\KitTransaction;
use App\Models\KitStock;
use Illuminate\Support\Facades\Validator;   // ← ADD THIS
use Illuminate\Support\Facades\DB;

class MasterKitController extends Controller
{
    public function index(Request $request){
        $user_type =  UserType::where('status', 1)->where('id', '!=', 1)->get();
        // $designayion_list =  MasterDesignation::latest()->get();

        $query = MasterKitNumber::select([
                'master_kit_numbers.*',
                'users.first_name as addedByName',
                'members.name as memberName',
                'l.first_name as leaderName',
                'vm.first_name as vivahMitraName',
            ])->join('users', 'users.id', '=', 'master_kit_numbers.created_by')
              ->leftJoin('members', 'members.id', '=', 'master_kit_numbers.member_id')
              ->leftJoin('users as l', 'l.id', '=', 'master_kit_numbers.leader_id')
              ->leftJoin('users as vm', 'vm.id', '=', 'master_kit_numbers.vivah_mitra_id');

            if ($request->vivah_mitra) {
                $query->where('master_kit_numbers.vivah_mitra_id', $request->vivah_mitra);
            }

            if ($request->search) {
                $search = $request->search;
                $query->where(function ($q) use ($search) {
                    $q->where('master_kit_numbers.kit_number', 'like', "%$search%")
                    ->orWhere('members.name', 'like', "%$search%");
                });
            }

            $membership_list = $query->orderBy('master_kit_numbers.id', 'DESC')->paginate(20);
            // dd($membership_list);
        $total_card = MasterKitNumber::count();
        $total_used_card = MasterKitNumber::where('is_used', 1)->count();
        $total_available_card = MasterKitNumber::where('is_used', 0)->count();
        $total_issue_cards = KitTransaction::where('type', 'issue')->where('from_user_id', NULL)->sum('quantity');
        ...
        $datas = [
            ...
            'page_title' => 'Generate Kit Numbers',
            'vivah_mitra_list' => User::where('user_type_id', 6)->get(),
        ];
        return view('admin.kit_numbers.index', $datas);
    }

    public function fetchKitData(Request $request)
    {
        $query = MasterKitNumber::select([
                'master_kit_numbers.*',
                'users.first_name as addedByName',
        ...
        if ($request->ajax()) {
            return view('admin.kit_numbers.table_ajax', compact('membership_list'))->render();
        }
    }
    ...
    public function store(Request $request){
        $request->validate([
            'number' => 'required|numeric|min:10|max:100',
        ...
        for ($i = 0; $i < $count; $i++) {
			...
			do {
				...
				$newCode = '14' . $random; // total = 8 digits

				$exists = MasterKitNumber::where('kit_number', $newCode)->exists();

			} while ($exists);

			MasterKitNumber::create([
				'kit_number'   => $newCode,
        ...
        return response()->json([
            'status' => true,
            'message'  => 'Kit Numbers generated successfully!',
...
*/
    ...
    public function kitTransferHistory(Request $request)
    {
        $page_title = "Kit Transfer History";
        $query = KitTransaction::select([
                'kit_transactions.*',
                'fu.first_name as from_user_name',
                'tu.first_name as to_user_name',
            ])->leftJoin('users as fu', 'fu.id', '=', 'kit_transactions.from_user_id')
              ->leftJoin('users as tu', 'tu.id', '=', 'kit_transactions.to_user_id');
            ...
            $kit_transactions = $query->orderBy('kit_transactions.id', 'DESC')->paginate(20);

            return view('admin.staffs.vivah_mitra.vivahmitra.kit_transfer_history', compact('kit_transactions', 'page_title'));

        if ($request->ajax()) {
            return view('admin.staffs.vivah_mitra.vivahmitra.kit_transfer_history_ajax', compact('kit_transactions'))->render();
        }
    }
